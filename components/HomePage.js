...
function HomePage({ navigateTo }) {
  ...
  const { restaurants, loading, error } = useSelector((state) => state.db);
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <TouchableOpacity onPress={() => navigateTo("DiscoverPage")}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              onPress={() =>
                navigateTo("RestaurantDetails", { restaurant: item })
              }
            >
              ...
              {item.image ? (
                ...
                >
                  <View style={styles.textOverlay}>
                    ...
                    {/* {item.reservationSlots &&
                      item.reservationSlots.length > 0 && (
                        <View style={styles.reservationContainer}>
                          <Text style={styles.reservationHeading}>
                            Reservation Slots:
                          </Text>
                          {item.reservationSlots.map((slot) => (
                            <Text key={slot._id} style={styles.reservationInfo}>
                              {moment(slot.date).format("lll")} - {slot.slots}{" "}
                              slots
                            </Text>
                          ))}
                        </View>
